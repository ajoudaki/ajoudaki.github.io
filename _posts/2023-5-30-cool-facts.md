...
- Very useful linear algebra identities:
  - $\tr(A^k)=\sum_{i=1}^n \lambda_i(A)^k$
  - $\tr((A^*A)^k) = \sum_{i=1}^n \sigma_i(A)^{2k}$
  - $\log\det(A-zI_n)=\sum_{i=1}^n \log|\lambda_i(A)-z|=\sum_{i=1}^n\log|\sigma_i(A-z I)|$
