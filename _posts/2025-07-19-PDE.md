...
Despite the quite beautiful PDE, it does not lead to solutions on its own and requires more information about the "physics" of the setup. 
...
With our key assumption in place—that the queue length $Q_k$ at an intersection follows an exponential distribution with mean $q_k$—we can derive the crucial relationship between the expected outflow $f_k$ and the expected queue length $q_k$. This relationship is the "closure relation" or "fundamental diagram" for our model.

The expected outflow at intersection $k$, which we denote $f(q)$ for a given expected queue length $q$, is a function of the intersection's own stochastic queue $Q$, its random capacity $X$, and the stochastic inflow from the previous intersection $Y$. We model both the normalized capacity and inflow as independent uniform random variables.
$$f(q) = E[\min(X, Q + Y)]$$
where $X \sim \text{Unif}[0,1]$, $Q \sim \text{Exp}(1/q)$, and $Y \sim \text{Unif}[0,1]$ are mutually independent.

While this expression seems daunting, we can derive its exact value and then find a simple, accurate approximation for the heavy-traffic regime ($q \to \infty$).

**Lemma: Expected Outflow**

*The exact expected outflow $f(q)$ is given by the expression:*
$$f(q) = \int_0^1 \left(\frac{1}{3} + \frac{z}{2} - \frac{z^2}{2} + \frac{z^3}{6}\right) \frac{1}{q}e^{-z/q} \,dz + \frac{1}{2} e^{-1/q}$$

> **Proof:**
> The expectation is taken over three random variables. We can compute it by first conditioning on the queue length $Q=z$ and finding the conditional expectation $g(z) = E_{X,Y}[\min(X, z+Y)]$.
> $$g(z) = \int_0^1 \int_0^1 \min(x, z+y) \,dx \,dy$$
> The value of this integral depends on whether $z$ is greater or less than 1.
> 1.  **Case 1: $z \ge 1$.** For any $y \in [0,1]$, we have $z+y \ge 1$. Since $x \in [0,1]$, it follows that $x \le z+y$. Therefore, $\min(x, z+y) = x$. The integral becomes $g(z) = \int_0^1 \int_0^1 x \,dx \,dy = \int_0^1 \frac{1}{2} \,dy = \frac{1}{2}$.
> 2.  **Case 2: $0 \le z < 1$.** We first integrate with respect to $x$. The integral over $x$ depends on the value of $z+y$:
>     $$ \int_0^1 \min(x, z+y)\,dx = \begin{cases} (z+y) - \frac{(z+y)^2}{2} & \text{if } z+y \le 1 \\ 1/2 & \text{if } z+y > 1 \end{cases} $$
>     Integrating this piecewise function over $y \in [0,1]$ gives:
     \begin{align}
     g(z) &= \int_0^{1-z} \left((z+y) - \frac{(z+y)^2}{2}\right) dy + \int_{1-z}^1 \frac{1}{2} dy \cr
     &= \left[\frac{(z+y)^2}{2} - \frac{(z+y)^3}{6}\right]_0^{1-z} + \frac{1}{2}(1-(1-z)) \cr
     &= \left(\frac{1}{2} - \frac{1}{6}\right) - \left(\frac{z^2}{2} - \frac{z^3}{6}\right) + \frac{z}{2} \cr
     &= \frac{1}{3} + \frac{z}{2} - \frac{z^2}{2} + \frac{z^3}{6}
     \end{align}
> Combining these cases, we have the exact conditional expectation $g(z)$. The total expectation $f(q)$ is found by averaging $g(z)$ over the exponential distribution of $Q$, which has PDF $p_Q(z) = (1/q)e^{-z/q}$:
> $$f(q) = E_Q[g(Q)] = \int_0^\infty g(z) \frac{1}{q}e^{-z/q} \,dz$$
> Splitting this integral at $z=1$ and substituting the expressions for $g(z)$ gives the result. The integral from $1$ to $\infty$ is $\int_1^\infty \frac{1}{2} \frac{1}{q}e^{-z/q} dz = \frac{1}{2}e^{-1/q}$, which completes the proof.

The exact formula is still too complex to be inserted into our PDE. However, we are interested in the heavy-traffic regime where queues are large ($q \to \infty$). In this limit, we can find a simple and elegant approximation.

**Corollary: Asymptotic Approximation for Heavy Traffic**

*In the heavy traffic limit as $q \to \infty$, the expected outflow $f(q)$ is given by the approximation:*
$$f(q) \approx \frac{1}{2} - \frac{1}{24q}$$

> **Proof:**
> We start from the exact expression $f(q) = E[g(Q)]$. We can rewrite this by separating the steady-state flow of $1/2$:
> $$f(q) = \int_0^\infty \frac{1}{2} p_Q(z)dz + \int_0^\infty \left(g(z) - \frac{1}{2}\right)p_Q(z)dz$$
> Since $\int_0^\infty p_Q(z)dz=1$ and $g(z)=1/2$ for $z \ge 1$, the second integral is non-zero only for $z \in [0,1]$. This simplifies to:
> $$f(q) = \frac{1}{2} + \int_0^1 \left(g(z) - \frac{1}{2}\right) \frac{1}{q}e^{-z/q} dz$$
> For large $q$, the term $e^{-z/q}$ is close to 1 for any $z$ in the integration range $[0,1]$. Thus, the leading-order behavior of the integral is given by setting $e^{-z/q} \approx 1$:
> $$ f(q) - \frac{1}{2} \approx \frac{1}{q} \int_0^1 \left(g(z) - \frac{1}{2}\right) dz $$
> We evaluate the definite integral:
\begin{align}
\int_0^1 &\left(\left(\frac{1}{3} + \frac{z}{2} - \frac{z^2}{2} + \frac{z^3}{6}\right) - \frac{1}{2}\right) dz \cr
&= \int_0^1 \left(-\frac{1}{6} + \frac{z}{2} - \frac{z^2}{2} + \frac{z^3}{6}\right) dz \cr
&= \left[-\frac{z}{6} + \frac{z^2}{4} - \frac{z^3}{6} + \frac{z^4}{24}\right]_0^1 \cr
&= -\frac{1}{6} + \frac{1}{4} - \frac{1}{6} + \frac{1}{24} \cr
&= \frac{-4+6-4+1}{24} = -\frac{1}{24}
\end{align}
> Substituting this result back, we arrive at the asymptotic relationship:
> $$ f(q) \approx \frac{1}{2} + \frac{1}{q} \left(-\frac{1}{24}\right) = \frac{1}{2} - \frac{1}{24q} $$
> This provides the accurate, tractable relationship we need.
...
= - \frac{d}{dq}\left(\frac{1}{2} - \frac{1}{24q}\right) \frac{\partial q}{\partial k}
= - \left(\frac{1}{24q^2}\right) \frac{\partial q}{\partial k}
$$
...
$$\frac{dq_1}{dt} = f_0 - f_1 = 1 - f(q_1) \approx 1 - \left(\frac{1}{2} - \frac{1}{24q_1}\right) = \frac{1}{2} + \frac{1}{24q_1}$$

Wait, let's re-check the boundary condition logic from the original post. The inflow to the first intersection is 1 (normalized). The outflow is $f_1(q_1)$. So $dq_1/dt = 1 - f_1(q_1)$. The original post used $f_0 = 1/2$. Why? The inflow at $k=0$ is deterministic and equals 1. $f_{-1}(t) \equiv 1$ was defined. The first *queued* intersection is $k=0$. Let's call it $k=1$ to match the post. Then the inflow is $f_0(t) \equiv 1$.
Then $\frac{dq_1}{dt} = 1 - f_1(q_1) \approx 1 - (\frac{1}{2} - \frac{1}{24q_1}) = \frac{1}{2} + \frac{1}{24q_1}$. For large $q_1$, this is $\frac{dq_1}{dt} \approx \frac{1}{2}$. So $q_1(t) \approx t/2$. This seems different from the original post's derivation $q_1(t)=\sqrt{t/12}$. Let's re-read the original model setup. Ah, `F_{-1}(t) \equiv 1`. But in the continuous model `f_0` is used. What is `f_0`? The model assumes a *stream* of intersections $k=0, 1, ...$. The inflow to the *first* intersection $k=0$ is 1. Its outflow is $f_0$. This $f_0$ is the inflow to intersection $k=1$. The queue $q_0$ is assumed to be 0 (no queue before the first intersection).
Then $f_0 = E[\min(X_0, Q_{-1} + F_{-1})] = E[\min(X_0, 0 + 1)] = E[X_0] = 1/2$.
So the inflow to the first queued intersection ($k=1$ in the post's notation) is indeed $f_0=1/2$. The original derivation was correct.
